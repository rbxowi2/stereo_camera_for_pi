...
config_a = picam2a.create_video_configuration(
    raw={"size": resolution, "format": "SBGGR10"},
...
frames_in_memory = np.empty(((n * b), frame_height, frame_width*4), dtype=np.uint8)
    ...
    while time.time() - start_time < duration:
        if frame_count < max_frames:
            ...
            if frame_b_ready and frame_a_ready:
                ...
                frames_in_memory[n_point][:, :(resolution[0]*2)] = picam2b.capture_array("raw")
                frames_in_memory[n_point][:, (resolution[0]*2):] = picam2a.capture_array("raw")
                ...
                if b_point_temp != b_point:
                    while lock_b[b_point_temp]:    # 阻塞等待新區塊釋放
                        drop_=frame_count
