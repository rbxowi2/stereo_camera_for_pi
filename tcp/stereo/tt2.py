...
if not os.path.exists(stereo_param_file):
    print(f"❌ 找不到校正參數檔案: {stereo_param_file}")
    exit(1)
...
# ========= 建立 rectify map using pinhole model =========
R1, R2, P1, P2, Q, roi1, roi2 = cv2.stereoRectify(
    K_l, D_l, K_r, D_r, DIM, R, T,
    flags=cv2.CALIB_ZERO_DISPARITY,
    alpha=0  # 0 = 去除黑邊、裁切，1 = 保留原圖
)

map1_l, map2_l = cv2.initUndistortRectifyMap(
    K_l, D_l, R1, P1, DIM, cv2.CV_16SC2
)
map1_r, map2_r = cv2.initUndistortRectifyMap(
    K_r, D_r, R2, P2, DIM, cv2.CV_16SC2
)

# ========= TCP 視訊串流 =========
server_ip = "192.168.200.42"  # 修改為你的伺服器 IP
server_port = 5000
