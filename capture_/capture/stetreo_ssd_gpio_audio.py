...
temp_filename = "/dev/shm/temp_recording.wav"
...
def start_audio_recording():
    global recording_process, record_start_time
    ...
    print(f"錄音開始時間：{record_start_time.isoformat(timespec='milliseconds')}")
...
def stop_audio_recording():
    ...
    if recording_process:
        ...
        # 使用錄音開始時間作為檔名
        timestamp = record_start_time.strftime("%Y%m%d_%H%M%S_%f")[:-3]  # 到毫秒
        final_wav = os.path.join(dirpath, f"recording_{timestamp}.wav")
        shutil.move(temp_filename, final_wav)
        print(f"錄音檔已儲存：{final_wav}")

        # 同步記錄開始時間資訊
        info_txt = os.path.join(dirpath, f"recording_{timestamp}.txt")
        with open(info_txt, "w") as f:
            f.write(f"Audio_Start Time: {record_start_time.isoformat(timespec='milliseconds')}\n")
            f.write("Format: S32_LE, Channels: 2, Rate: 44100Hz\n")
            for i, timestamp in enumerate(frames_timestamp):
                frame_number = f"{i:04d}"  # 將frame編號格式化為4位數
    ...
    while True:
        if record_flag == 1 :
            if frame_b_ready and frame_a_ready:
                ...
                frames_timestamp.append(datetime.datetime.now().isoformat(timespec='milliseconds'))
