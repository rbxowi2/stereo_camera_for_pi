...
for idx, raw_file in enumerate(raw_files):
    ...
    # 我們把所有3D點在 Z 方向上「減去」 target_eye_relief（簡單模擬眼睛向後移）
    # 實際上：若 target_eye_relief > 0 (眼睛離鏡片更遠)，我們要把 3D 點相對 camera 向近看起來更遠
    ...
    # 因為我們要產生左右眼的 equirect 畫面（兩張），對左眼採樣 rect_l；對右眼採樣 rect_r
    # 注意：若你想要左右眼在 Eye Relief 方向再做微調（例如左右偏移IPD），可以在此額外平移 X。
